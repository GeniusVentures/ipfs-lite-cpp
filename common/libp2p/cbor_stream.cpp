...
{
    CborStream::CborStream( std::shared_ptr<Stream> stream ) : stream_{ std::move( stream ) } {}
    ...
    void CborStream::writeRaw( gsl::span<const uint8_t> input, WriteCallbackFunc cb )
    {
    ...
    void CborStream::readMore( ReadCallbackFunc cb )
    {
        ...
        stream_->readSome( gsl::make_span( buffer_ ).subspan( size_ ),
                           kReserveBytes,
