...
namespace sgns::ipfs_lite::ipfs::graphsync
{
    ...
    namespace
    {
        ...
        std::map<std::string, CborEncodeStream> encodeMetadataItem( const std::pair<CID, bool> &item )
        {
        ...
        IPFS::outcome::result<bool> decodeBool( const Container &s )
        {
            ...
            logger()->warn( "{}: wrong cbor encoding: single byte expected", __FUNCTION__ );
            return Error::MESSAGE_PARSE_ERROR;
        }
    ...
    {
        static const Extension e = { std::string( kResponseMetadataProtocol ), std::vector<uint8_t>{ 0xF5 } };
        return e;
    }
    ...
    IPFS::outcome::result<ResponseMetadata> decodeResponseMetadata( const Extension &extension )
    {
        ...
        try
        {
            ...
            for ( size_t i = 0; i < n; ++i )
            {
                ...
                if ( !x.isMap() )
                {
                    logger()->warn( "{}: wrong cbor encoding: entry is not a map", __FUNCTION__ );
                    return Error::MESSAGE_PARSE_ERROR;
                }
                ...
                {
                    logger()->warn( "{}: wrong cbor encoding: required fields missing", __FUNCTION__ );
                    return Error::MESSAGE_PARSE_ERROR;
                }
    ...
    IPFS::outcome::result<std::set<CID>> decodeDontSendCids( const Extension &extension )
    {
        ...
        return std::set<CID>( std::move_iterator( cids.begin() ), std::move_iterator( cids.end() ) );
    }
