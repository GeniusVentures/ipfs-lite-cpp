...
namespace sgns::ipfs_lite::ipfs::graphsync {
  ...
  void PeerContext::onStreamConnected(IPFS::outcome::result<StreamPtr> rstream) {
    ...
    if (rstream) {
      ...
      logger()->error(
          "cannot connect, peer={}, msg='{}' state={}", str, rstream.error().message(), static_cast<int>(getState()));
      if (getState() == is_connecting) {
        // Close the entire PeerContext, not just local requests
  ...
  void PeerContext::enqueueRequest(RequestId request_id,
    ...
    if (closed_) {
      logger()->warn("enqueueRequest: PeerContext is already closed for peer {}, state={}, calling onResponse with RS_INTERNAL_ERROR", 
                    str, static_cast<int>(getState()));
    ...
    if (!requests_endpoint_) {
      logger()->error("enqueueRequest: Internal error, state={}, calling close()", static_cast<int>(getState()));
      close(RS_INTERNAL_ERROR);
      return;
    }
    ...
    if (res) {
      ...
      logger()->debug("enqueueRequest: request_id {} added to local_request_ids_ for peer {}, state={}", 
                     request_id, str, static_cast<int>(getState()));
    } else {
