...
namespace sgns::ipfs_lite
{
    ...
    IPFS::outcome::result<void> rocksdb::Batch::put( const Buffer &key, const Buffer &value )
    {
        batch_.Put( make_slice( key ), make_slice( value ) );
        return IPFS::outcome::success();
    }

    IPFS::outcome::result<void> rocksdb::Batch::put( const Buffer &key, Buffer &&value )
    {
