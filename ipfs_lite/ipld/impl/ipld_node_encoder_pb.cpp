...
namespace sgns::ipfs_lite::ipld
{
                                                    ...
                                                    const std::set<std::string>           &destinations )
    {
        ...
        result.insert( result.end(), std::move_iterator( links_pb.begin() ), std::move_iterator( links_pb.end() ) );
        result.insert( result.end(), std::move_iterator( content_pb.begin() ), std::move_iterator( content_pb.end() ) );
    ...
    std::vector<uint8_t> IPLDNodeEncoderPB::serializeLinks( const std::map<std::string, IPLDLink> &links )
    {
        ...
        if ( links_content_size > 0 )
        {
            ...
            for ( const auto &link : links )
            {
                ...
                PBTag cid_tag = createTag( PBFieldType::LENGTH_DELEMITED, static_cast<uint8_t>( PBLinkOrder::HASH ) );
                ...
                // Write link name:
                PBTag name_tag = createTag( PBFieldType::LENGTH_DELEMITED, static_cast<uint8_t>( PBLinkOrder::NAME ) );
                ...
                PBTag size_tag = createTag( PBFieldType::VARINT, static_cast<uint8_t>( PBLinkOrder::SIZE ) );
    ...
    std::vector<uint8_t> IPLDNodeEncoderPB::serializeContent( const common::Buffer &content )
    {
        ...
        {
            ArrayOutputStream array_output_stream( buffer.data(), buffer.size() );
            PBTag data_tag = createTag( PBFieldType::LENGTH_DELEMITED, static_cast<uint8_t>( PBNodeOrder::DATA ) );
            CodedOutputStream coded_stream{ &array_output_stream };
    ...
    std::vector<uint8_t> IPLDNodeEncoderPB::serializeDestinations( const std::set<std::string> &destinations )
    {
    ...
    size_t IPLDNodeEncoderPB::getDestinationsLengthPB( const std::set<std::string> &destinations )
    {
        size_t length{};
        for ( const auto &destination : destinations )
        {
    ...
    IPLDNodeEncoderPB::PBTag IPLDNodeEncoderPB::createTag( PBFieldType type, uint8_t order )
    {
