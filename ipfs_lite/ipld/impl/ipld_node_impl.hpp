...
namespace sgns::ipfs_lite::ipld
{
    ...
    public:
        ...
        IPFS::outcome::result<void> addChild( const std::string &name, std::shared_ptr<const IPLDNode> node ) override;

        IPFS::outcome::result<std::reference_wrapper<const IPLDLink>> getLink( const std::string &name ) const override;

        void removeLink( const std::string &name ) override;

        void addLink( const IPLDLink &link ) override;

        std::vector<std::reference_wrapper<const IPLDLink>> getLinks() const override;

        Buffer serialize() const override;

        static std::shared_ptr<IPLDNode> createFromString( const std::string &content );

        static IPFS::outcome::result<std::shared_ptr<IPLDNode>> createFromRawBytes( gsl::span<const uint8_t> input );
    ...
    {
        return { IPLDType::Version::V0, IPLDType::Content::DAG_PB, IPLDType::Hash::sha256 };
    }

    template <>
    inline common::Buffer IPLDBlock::serialize<IPLDNodeImpl>( const IPLDNodeImpl &entity )
    {
