...
namespace sgns::ipfs_lite::ipld::walker
{
    ...
    {
        static IPFS::outcome::result<std::vector<CID>> links( gsl::span<const uint8_t> input )
        {
            std::vector<CID> cids;
            PbDecoder        s{ input };
            while ( true )
    ...
    IPFS::outcome::result<void> Walker::select( const CID &root, const Selector &selector )
    {
