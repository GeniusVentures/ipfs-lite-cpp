#include "ipld_node.hpp"

#include "ipfs_lite/ipld/impl/ipld_node_decoder_pb.hpp"
#include "ipfs_lite/ipld/impl/protobuf/ipld_node.pb.h"

namespace sgns::ipfs_lite::ipld {

  const CID &IPLDNode::getCID() const {
    return getIPLDBlock().cid;
  }

  const IPLDNode::Buffer &IPLDNode::getRawBytes() const {
    return getIPLDBlock().bytes;
  }

  size_t IPLDNode::size() const {
    return getRawBytes().size() + child_nodes_size_;
  }

  void IPLDNode::assign(common::Buffer input) {
    content_ = std::move(input);
    ipld_block_ = std::nullopt;  // Need to recalculate CID after changing Node content
  }

  IPFS::outcome::result<void> IPLDNode::addChild(
      const std::string &name, const IPLDNode& node) {
      IPLDLink link{node.getCID(), name, node.size()};
      links_.emplace(name, std::move(link));
      ipld_block_ = std::nullopt; // Need to recalculate CID after adding link to child node
      child_nodes_size_ += node.size();
      return IPFS::outcome::success();
  }

  IPFS::outcome::result<std::reference_wrapper<const IPLDLink>> IPLDNode::getLink(
      const std::string &name) const {
    if (auto index = links_.find(name); index != links_.end()) {
      return index->second;
    }
    return IPLDNodeError::LINK_NOT_FOUND;
  }

  void IPLDNode::removeLink(const std::string &link_name) {
    if (auto index = links_.find(link_name); index != links_.end()) {
      child_nodes_size_ -= index->second.getSize();
      links_.erase(index);
    }
  }

  void IPLDNode::addLink(IPLDLink link) {
    links_.emplace(link.getName(), std::move(link));
  }

  std::vector<std::reference_wrapper<const IPLDLink>> IPLDNode::getLinks()
      const {
    std::vector<std::reference_wrapper<const IPLDLink>> link_refs{};
    link_refs.reserve(links_.size());
    for (const auto &link : links_) {
      link_refs.emplace_back(link.second);
    }
    return link_refs;
  }

  IPLDNode::Buffer IPLDNode::serialize() const {
    return Buffer{IPLDNodeEncoderPB::encode(content_, links_)};
  }

  std::shared_ptr<IPLDNode> IPLDNode::createFromString(
      const std::string &content) {
    std::vector<uint8_t> data{content.begin(), content.end()};
    auto node = std::make_shared<IPLDNode>();
    node->assign(common::Buffer{data});
    return node;
  }

  IPFS::outcome::result<std::shared_ptr<IPLDNode>> IPLDNode::createFromRawBytes(
      gsl::span<const uint8_t> input) {
    IPLDNodeDecoderPB decoder;
    if (auto result = decoder.pb_node_.ParseFromArray(input.data(), input.size()); !result) {
      return IPLDNodeDecoderPBError::INVALID_RAW_BYTES;
    }

    auto node = createFromString(decoder.getContent());
    for (size_t i = 0; i < decoder.getLinksCount(); ++i) {
      std::vector<uint8_t> link_cid_bytes{decoder.getLinkCID(i).begin(),
                                          decoder.getLinkCID(i).end()};
      BOOST_OUTCOME_TRY(auto && link_cid, CID::fromBytes(link_cid_bytes));
      IPLDLink link{std::move(link_cid), decoder.getLinkName(i), decoder.getLinkSize(i)};
      node->addLink(link);
    }
    return node;
  }

  const IPLDBlock &IPLDNode::getIPLDBlock() const {
    if (!ipld_block_) {
      ipld_block_ = IPLDBlock::create(*this);
    }
    return ipld_block_.value();
  }
}  // namespace sgns::ipfs_lite::ipld

OUTCOME_CPP_DEFINE_CATEGORY_3(sgns::ipfs_lite::ipld, IPLDNodeError, e) {
  using sgns::ipfs_lite::ipld::IPLDNodeError;
  switch (e) {
    case (IPLDNodeError::LINK_NOT_FOUND):
      return "MerkleDAG Node: link not exist";
    case (IPLDNodeError::INVALID_RAW_DATA):
      return "MerkleDAG Node: failed to deserialize from incorrect raw bytes";
  }
  return "MerkleDAG Node: unknown error";
}
