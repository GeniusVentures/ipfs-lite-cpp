...
namespace sgns::data_transfer
{
    ...
    IPFS::outcome::result<void> MessageReceiver::registerVoucherType( const std::string                &type,
    ...
    IPFS::outcome::result<void> MessageReceiver::validateVoucher( const PeerId              &sender,
                                                                  const DataTransferRequest &request ) const
    {
        ...
        if ( request.is_pull )
        {
            BOOST_OUTCOME_TRYV2( auto &&,
                                 validator->second->validatePull( sender, request.voucher, base_cid, selector ) );
        }
        else
        {
            BOOST_OUTCOME_TRYV2( auto &&,
                                 validator->second->validatePush( sender, request.voucher, base_cid, selector ) );
        }
