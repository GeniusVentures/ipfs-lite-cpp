...
#include "core/ipfs_lite/ipfs/merkledag/ipfs_merkledag_dataset.hpp"
#include "ipfs_lite/ipfs/impl/in_memory_datastore.hpp"
#include "ipfs_lite/ipfs/impl/ipfs_block_service.hpp"

using namespace sgns::ipfs_lite::ipfs;
using namespace sgns::ipfs_lite::ipld;
using namespace sgns::ipfs_lite::ipfs::merkledag;
...
TEST_P( CommonFeaturesTest, GetNodeSuccess )
{
    ...
    {
        EXPECT_OUTCOME_TRUE( received_node, merkledag_service_->getNode( node->getCID() ) )
...
TEST_P( CommonFeaturesTest, GraphSyncSelect )
{
    ...
    EXPECT_OUTCOME_TRUE( selected_count, merkledag_service_->select( root_cid, gsl::span<const uint8_t>{}, handler ) );
...
INSTANTIATE_TEST_CASE_P(
    ...
    CommonFeaturesTest,
    testing::Values(
        DataSample{ dataset::getTypeA(), "QmdfTbBqBPQ7VNxZEYEj14VmRuZBkqFbiwReogJgS1zR1n", "{[]}" },
        DataSample{ dataset::getTypeB(), "QmTq5KSpqFrzJTQ7LCDCr7GmKZrWW46pp2DSrW3WibgFV6", "{[leve1_node1]}" },
        DataSample{ dataset::getTypeC(), "Qmaybnje7u6r2suDoUehBqxJU8dQ3MKwrUJ5Bi7qt68rDg", "{[]->{[leve1_node1]}}" },
        DataSample{ dataset::getTypeD(),
