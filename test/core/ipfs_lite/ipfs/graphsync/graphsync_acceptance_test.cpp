...
namespace sgns::ipfs_lite::ipfs::graphsync::test
{
    ...
    public:
                          ...
                          const CID                                   &root_cid )
        {
            ...
            Extension              response_metadata_extension = encodeResponseMetadata( response_metadata );
            ...
            //                                             extensions,
            //                                             requestProgressCallback());
            requests_.push_back( std::shared_ptr<Subscription>(
                new Subscription( std::move( graphsync_->makeRequest( peer,
                                                                      std::move( address ),
                                                                      root_cid,
                                                                      {},
                                                                      extensions,
    ...
    private:
        ...
        {
            static auto formatExtensions = []( const std::vector<Extension> &extensions ) -> std::string
            {
                std::string s;
    ...
    void testManyNodesExchange( size_t N, size_t n_data )
    {
        ...
        for ( size_t i = 0; i < N; ++i )
        {
            ...
            p.listen_to = libp2p::multi::Multiaddress::create( fmt::format( "/ip4/127.0.0.1/tcp/{}", 40000 + i ) )
                              .value();
        ...
        for ( size_t i = 0; i < N; ++i )
        {
            ...
            auto cb =
                [ds = p.data_service, &expected, &unexpected_responses, &total_responses, &io]( CID            cid,
                                                                                                common::Buffer data )
            {
...
{
    namespace test = sgns::ipfs_lite::ipfs::graphsync::test;
