...
auto kCidRaw  = "122031C3D57080D8463A3C63B2923DF5A1D40AD7A73EAE5A14AF584213E5F504AC33"_cid;
...
TEST( Cbor, Map )
{
    ...
    EXPECT_OUTCOME_EQ( ( decode<std::map<std::string, int>>( "A3636F6E65016374776F0265746872656503"_unhex ) ), m );
}
...
TEST( CborDecoder, CidErrors )
{
    ...
    EXPECT_OUTCOME_ERROR(
        ...
        decode<CID>( "582300122031C3D57080D8463A3C63B2923DF5A1D40AD7A73EAE5A14AF584213E5F504AC33"_unhex ) );
    // not 42 tag
    EXPECT_OUTCOME_ERROR(
        CborDecodeError::INVALID_CBOR_CID,
        decode<CID>( "D82B582300122031C3D57080D8463A3C63B2923DF5A1D40AD7A73EAE5A14AF584213E5F504AC33"_unhex ) );
    // empty 42 tag
    EXPECT_OUTCOME_ERROR( CborDecodeError::INVALID_CBOR, decode<CID>( "D82A"_unhex ) );
    // not bytes
    EXPECT_OUTCOME_ERROR( CborDecodeError::INVALID_CBOR_CID, decode<CID>( "D82B01"_unhex ) );
    // no multibase 00 prefix
    EXPECT_OUTCOME_ERROR(
        CborDecodeError::INVALID_CBOR_CID,
        decode<CID>( "D82A5822122031C3D57080D8463A3C63B2923DF5A1D40AD7A73EAE5A14AF584213E5F504AC33"_unhex ) );
    // invalid cid
    EXPECT_OUTCOME_ERROR( CborDecodeError::INVALID_CID, decode<CID>( "D82A420000"_unhex ) );
}
...
{
    sgns::IPFS::outcome::result<std::vector<uint8_t>> resolve( gsl::span<const uint8_t> node, const std::string &part )
    {
...
{
    EXPECT_OUTCOME_ERROR( CborResolveError::CONTAINER_EXPECTED, resolve( kCidCbor, "a" ) );
}
