...
using namespace sgns::primitives::cid;
...
TEST_F(CidJsonTest, EncodeAndPrint) {
  EXPECT_OUTCOME_TRUE(value, sgns::codec::json::encodeCidVector(cids));
  stripString(value);
  ASSERT_EQ(value, json_value);
  EXPECT_OUTCOME_TRUE(res, sgns::codec::json::decodeCidVector(value));
  ASSERT_EQ(res, cids);
}
