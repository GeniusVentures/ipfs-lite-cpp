...
{
    CborEncodeStream &CborEncodeStream::operator<<( const std::vector<uint8_t> &bytes )
    {
        return *this << gsl::make_span( bytes );
    }

    CborEncodeStream &CborEncodeStream::operator<<( gsl::span<const uint8_t> bytes )
    {
        addCount( 1 );
        ...
        data_.insert( data_.end(),
                      encoded.begin(),
                      encoded.begin() + cbor_encoder_get_buffer_size( &encoder, encoded.data() ) );
        return *this;
    }
    ...
    CborEncodeStream &CborEncodeStream::operator<<( const CborEncodeStream &other )
    {
    ...
    CborEncodeStream &CborEncodeStream::operator<<( const std::map<std::string, CborEncodeStream> &map )
    {
        ...
        data_.insert( data_.end(),
                      prefix.begin(),
                      prefix.begin() + cbor_encoder_get_buffer_size( &container, prefix.data() ) );
    ...
    std::vector<uint8_t> CborEncodeStream::data() const
    {
        ...
        std::vector<uint8_t> result( prefix.begin(),
                                     prefix.begin() + cbor_encoder_get_buffer_size( &container, prefix.data() ) );
    ...
    CborEncodeStream CborEncodeStream::wrap( gsl::span<const uint8_t> data, size_t count )
    {
        CborEncodeStream s;
