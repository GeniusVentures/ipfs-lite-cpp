...
namespace sgns::codec::cbor
{
    ...
    {
        if ( CborNoError != cbor_parser_init( data_->data(), data_->size(), 0, parser_.get(), &value_ ) )
        {
    ...
    void CborDecodeStream::next()
    {
        ...
        if ( value_.ptr != parser_->end )
        {
            ...
            if ( CborNoError != cbor_parser_init( value_.ptr, parser_->end - value_.ptr, 0, parser_.get(), &value_ ) )
            {
                IPFS::outcome::raise( CborDecodeError::INVALID_CBOR );
            }
    ...
    std::map<std::string, CborDecodeStream> CborDecodeStream::map()
    {
        ...
        while ( !cbor_value_at_end( &stream.value_ ) )
        {
            ...
            if ( CborNoError !=
                 cbor_parser_init( begin, stream.value_.ptr - begin, 0, stream2.parser_.get(), &stream2.value_ ) )
            {
                IPFS::outcome::raise( CborDecodeError::INVALID_CBOR );
            }
